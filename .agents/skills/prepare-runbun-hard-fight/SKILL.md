...
5. Prepare the team.
   ...
   - After party setup, generate the schema-v2 fight profile with `game_battle_profile`. Do not hand-copy species/move branches from an earlier trainer. Generated matchup assignments replace manual forced-switch and reserve tables; add an exception only when a reviewed bounded replay proves the generic scorer and reusable strategies cannot express the matchup.
...
7. Validate readiness.
   ...
   - Run one bounded precontact attempt with `tools/run_battle_policy.py --state CHECKPOINT --profile runtime/session/policy_profiles/PROFILE.json --trainer-local-id ID`. Schema-v2 clones automatically trial matching candidate strategies; the identical manifest is frozen into the behavior hash used for qualified live execution. Omit `--trainer-local-id` only when the checkpoint is already at an active battle decision. The runner rejects an overworld state without an atomic trainer target before creating a review, persists every verified transition, saves bounded counterfactual probe states when action-changing uncertainty is detected, writes a postmortem, and updates the clone qualification ledger before another attempt is allowed.
